wow_feedreader = AceLibrary("AceAddon-2.0"):new("AceEvent-2.0")

local debug = 0
function wow_feedreader:OnInitialize()
end

function wow_feedreader:OnEnable()
   self:RegisterEvent("RequestUpdates")
   -- Ensure that the saved variables are loaded
   self:RegisterEvent("ADDON_LOADED")
end

function wow_feedreader:ADDON_LOADED()
   if(debug == 1) then
...
function wow_feedreader:RequestUpdates()
